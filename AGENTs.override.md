...
- `docs/CODE_UNDERSTANDING.md`: a Markdown document written from scratch that records the agent's current understanding of how the model is actually implemented in code, including practical simplifications, implementation-specific choices, and open questions.
...
Maintain `docs/CODE_UNDERSTANDING.md` as a living checkpoint of the agent's understanding of the actual implementation. Use it to summarize how the model is realized in BrainPy, where the implementation differs from the theory, and which code-level questions remain unresolved. Revise it when code understanding improves or when the user clarifies implementation intent.
