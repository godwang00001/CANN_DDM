...
def _model_params(model) -> dict:
    return {
        ...
        "decision_space_params": {
            ...
            "decision_paradigm": getattr(model, "decision_paradigm", "free_response"),
            "t_start": int(model.t_start),
            "dur": int(model.dur),
            "max_time": int(getattr(model, "max_time", model.dur)),
            ...
            "lambda_click_L": float(getattr(model, "lambda_click_L", 0.0)),
            "lambda_click_R": float(getattr(model, "lambda_click_R", 0.0)),
            "delta_click_x": float(getattr(model, "delta_click_x", 0.0)),
            "x0": float(model.x0),
            "seed": model.seed,
        },
    }
