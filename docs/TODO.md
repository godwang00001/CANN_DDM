...
- Update the Figure 3 notebooks to load the new one-condition combined DDM+circuit bundle launched through `scripts/submit_fig3_one_condition_scc.sh`.
- Extend the current Figure 3 DDM-only macroscopic panel into the corresponding circuit-vs-cDDM comparison panel rather than leaving the new notebook as a one-model visualization only.
- Decide whether the active Figure 3 two-condition DDM generator should keep its current hard-coded `n=3000` condition block or be generalized / renamed so the default `--run-name` no longer implies `n1000`.
