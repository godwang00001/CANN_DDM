...
def _stack_time_ms(*time_axes: np.ndarray) -> np.ndarray:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    time_ms = _stack_time_ms(ddm_sweep.time_ms, circuit_sweep.time_ms)
    ...
    with (output_root / summary_name).open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=merged_fieldnames(summary_rows))
        writer.writeheader()
