...
def apply_figure_drive(model: CANN_DDM_model) -> None:
    t_start = int(model.t_start)
    t1 = 200
    t2 = 400
    model.v_drift_all[:] = 0.0
    model.v_drift_all[t_start:t_start + t1] = 1.0
    model.v_drift_all[t_start + t1:t_start + t2] = -1.0
    model.v_drive_all[:] = model.v_drift_all + model.v_noise_all


def summarize_run(model: CANN_DDM_model) -> dict:
    apply_figure_drive(model)
